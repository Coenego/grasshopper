...
/**
 * Get the calendar for an organising user
 *
 * @param  {Context}    ctx                 Standard context containing the current user and the current app
 ...
 * @param  {Event[]}    callback.events     The events in a user's calendar
 */
var getUserOrganising = module.exports.getUserOrganising = function(ctx, id, callback) {
    EventsDAO.getOrganiserEvents(id, function(err, events) {
        if (err) {
            return callback(err);
        }
