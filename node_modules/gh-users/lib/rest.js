...
/**
 * @REST getUserOrganising
 *
 * Get the events for an organiser
 *
 * @Server      admin,app
 * @Method      GET
 * @Path        /users/{id}/organising
 * @PathParam   {number}            id                      The id of the organising user to get the events for
 * @Return      {EventList}
 */
var _getUserOrganising = function(req, res) {
    UsersAPI.getUserOrganising(req.ctx, req.params.id, function(err, calendar) {
        if (err) {
            return res.status(err.code).send(err.msg);
        }
...
GrassHopper.globalAdminRouter.on('get', '/api/users/:id/organising', _getUserOrganising);
GrassHopper.appRouter.on('get', '/api/users/:id/organising', _getUserOrganising);
