...
var getEvent = module.exports.getEvent = function(id, callback) {
    ...
    DB.Event.findOne(options).complete(function(err, event) {
        if (err) {
...
/**
 * Get the events for an organiser
 *
 * @param  {Number}     id                  The id of the organiser for which to get the events
 * @param  {Function}   callback            Standard callback function
 * @param  {Object}     callback.err        An error object, if any
 * @param  {Event[]}    callback.events     The organisers of the event
 */
var getOrganiserEvents = module.exports.getOrganiserEvents = function(id, callback) {
    var sequelize = DB.getSequelize();

    var _query = "SELECT \"E\".* FROM \"Event_Organisers\" AS \"EO\" RIGHT JOIN \"Events\" AS \"E\" ON (\"EO\".\"EventId\" = \"E\".\"id\") WHERE \"EO\".\"UserId\" = " + id;

    sequelize.query(_query).complete(function(err, results) {
        if (err) {
            return callback({'code': 500, 'msg': 'Internal Server Error'});
        }
